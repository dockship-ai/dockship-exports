        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href={`tel:${COMPANY.phones[0]}`}
               className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white border border-white/25 hover:border-white/60 hover:bg-white/10 transition-all duration-200"
        ...
        {isMenuOpen && (
          <div className="lg:hidden py-4 border-t border-white/10"
            ...
            <div className="flex flex-col gap-1" aria-label="Mobile navigation">
              {NAV_LINKS.map((link) => (
                <button key={link.href} type="button" data-ocid="nav.link"
                        onClick={() => handleNavClick(link.href)}
                        className="text-left px-4 py-2.5 text-sm font-medium text-white/80 hover:text-white hover:bg-white/10 rounded-lg transition-all duration-200">
                  {link.label}
                </button>
              ))}
